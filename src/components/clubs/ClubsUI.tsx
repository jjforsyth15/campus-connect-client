...
export default function ClubsUI({ clubs, mode, club }: Props) {
      ...
      <Shell>
        <GlassPanel>
          <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 2, flexWrap: "wrap" }}>
            <Box>
              <Typography sx={{ color: "white", fontSize: { xs: 36, md: 46 }, fontWeight: 900 }}>Clubs Hub</Typography>
              <Typography sx={{ color: "rgba(255,255,255,0.70)", mt: 1, maxWidth: 700 }}>
                Discover CSUN communities — join clubs, find events, connect with members.
              </Typography>
        ...
        <Box sx={{ mt: 3 }}>
          <GlassPanel>
            <Box sx={{ display: "flex", justifyContent: "space-between", gap: 2, flexWrap: "wrap" }}>
              <Box>
                ...
                <Typography sx={{ color: "rgba(255,255,255,0.70)", mt: 0.5 }}>
                  Search by name, category, or vibe.
                </Typography>
              </Box>
              <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
            ...
            <Box sx={{ mt: 2.5, display: "grid", gridTemplateColumns: { xs: "1fr", md: "2fr 1fr 170px" }, gap: 1.5, alignItems: "center" }}>
              <TextField value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search clubs..." fullWidth InputProps={{ sx: inputSx }} />
              <Select value={cat} onChange={(e) => setCat(String(e.target.value))} fullWidth sx={selectSx}>
                <MenuItem value="All">All categories</MenuItem>
                <MenuItem value="Computer Science">Computer Science</MenuItem>
                <MenuItem value="Engineering">Engineering</MenuItem>
                <MenuItem value="Creative Tech">Creative Tech</MenuItem>
              </Select>
              <Button sx={btnPrimaryWide} onClick={() => document.getElementById("clubs-grid")?.scrollIntoView({ behavior: "smooth" })}>
                Browse →
              </Button>
            </Box>
          </GlassPanel>
          ...
          {filtered.map((c) => (
            ...
            >
                ...
                <CardContent sx={{ p: 3 }}>
                  <Box sx={{ display: "flex", justifyContent: "space-between", gap: 2 }}>
                    <Box>
                      ...
                      {c.contact?.email && (
                        <Typography sx={{ mt: 0.8, color: "rgba(42,0,16,0.60)", fontSize: 14 }}>{c.contact.email}</Typography>
                      )}
                    </Box>
              ...
              <Box sx={{ px: 3, pb: 3, pt: 0.5, display: "flex", gap: 1.2, flexWrap: "wrap" }}>
                <Button sx={btnMaroon} onClick={() => router.push(`/clubs?slug=${c.slug}`)}>View Club</Button>
                <Button sx={btnBlack} onClick={() => alert("Join flow next (DB + roles)")}>Join</Button>
                <Button sx={btnOutline} onClick={() => alert("Contact flow next")}>Contact</Button>
              </Box>
            </Card>
          ))}
        </Box>
  ...
  const tabsList = ["About", "Profile", "Members", "Roles", "Posts", "Apply", "Applications"] as const;
      ...
      <GlassPanel>
        <Box sx={{ display: "flex", justifyContent: "space-between", gap: 2, flexWrap: "wrap" }}>
          <Box>
            ...
            <Typography sx={{ color: "white", fontSize: 40, fontWeight: 900, mt: 0.5 }}>{club.name}</Typography>
          ...
          <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
            <Button component={Link} href="/clubs" sx={btnGhost}>← Back</Button>
            <Button sx={btnPrimary} onClick={() => alert("Next: Join flow")}>Join</Button>
          </Box>
        </Box>
      </GlassPanel>
        ...
        <GlassPanel>
          ...
          {tab === "Profile" && (
            <WhiteCard title="Club Profile">
              <Typography sx={{ color: "rgba(42,0,16,0.70)" }}>Profile foundation for clubs. Backend + permissions next.</Typography>
            </WhiteCard>
          )}
          {tab === "Members" && (
            <WhiteCard title="Members">
              <Typography sx={{ color: "rgba(42,0,16,0.70)" }}>Member management foundation. Backend + permissions next.</Typography>
            </WhiteCard>
          )}
          {tab === "Roles" && (
            <WhiteCard title="Roles">
              <Typography sx={{ color: "rgba(42,0,16,0.70)" }}>Appoint roles (President, VP, Treasurer). Permissions later.</Typography>
            </WhiteCard>
          )}
          {tab === "Posts" && (
            <WhiteCard title="Club Posts">
              <Typography sx={{ color: "rgba(42,0,16,0.70)" }}>Posts feed foundation. Next: DB + editor.</Typography>
            </WhiteCard>
          )}
          {tab === "Apply" && (
            <WhiteCard title="Apply to Join">
              <Typography sx={{ color: "rgba(42,0,16,0.70)" }}>Application foundation. Next: save to DB.</Typography>
            </WhiteCard>
          )}
          {tab === "Applications" && (
            <WhiteCard title="Applications">
              <Typography sx={{ color: "rgba(42,0,16,0.70)" }}>Admin view for reviewing applications. Permissions later.</Typography>
            </WhiteCard>
          )}
        </GlassPanel>
      </Box>
...
function ContactRow({ label, value }: { label: string; value?: string }) {
  return (
    <Box sx={{ mt: 2 }}>
      <Typography sx={{ fontSize: 12, fontWeight: 900, letterSpacing: 2, color: MAROON }}>{label.toUpperCase()}</Typography>
      <Typography sx={{ mt: 0.5, color: "rgba(42,0,16,0.75)" }}>
        {value ? (value.startsWith("http") ? <a href={value} target="_blank" rel="noreferrer" style={{ color: MAROON }}>{value}</a> : value) : <span style={{ color: "rgba(42,0,16,0.35)" }}>Not added yet</span>}
      </Typography>
