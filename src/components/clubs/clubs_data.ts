...
// Lightweight card data for the hub grid
// BACKEND: SELECT * FROM clubs ORDER BY name
export type Club = {
  id: string;
  slug?: string;
  name: string;
...
// BACKEND: SELECT * FROM club_members WHERE club_id = :clubId ORDER BY role
export type ClubMember = {
  id: string;
  name: string;
  avatar?: string;        // BACKEND: Supabase Storage — club_members.avatar_url
...
// BACKEND: SELECT * FROM club_posts WHERE club_id = :clubId ORDER BY created_at DESC
export type ClubPost = {
  id: string;
  author: string;
  authorAvatar?: string;  // BACKEND: Supabase Storage — users.avatar_url
  content: string;
  images?: string[];      // BACKEND: Supabase Storage — club_post_images.url[]
  createdAt: string;
  likes: number;
  liked: boolean;         // BACKEND: EXISTS (SELECT 1 FROM club_post_likes WHERE user_id = :userId AND post_id = id)
  comments: { id: string; author: string; text: string }[];
};

// BACKEND: SELECT * FROM club_events WHERE club_id = :clubId ORDER BY date ASC
export type ClubEvent = {
  ...
  time: string;
  location: string;
  cover?: string;         // BACKEND: Supabase Storage — club_events.cover_url
  rsvpCount: number;      // BACKEND: COUNT(*) FROM club_event_rsvps WHERE event_id = id
  rsvped: boolean;        // BACKEND: EXISTS (SELECT 1 FROM club_event_rsvps WHERE user_id = :userId AND event_id = id)
};

// BACKEND: SELECT DISTINCT category FROM clubs ORDER BY category
export const CLUB_CATEGORIES: ClubCategory[] = [
  "STEM", "Business", "Arts", "Cultural",
  "Sports", "Literature", "Fraternity", "Sorority",
];

// BACKEND: Replace with: const clubs = await supabase.from('clubs').select('*').order('name')
