...
//
// This file also overrides the site-wide background-color set in globals.css
// so that the social feed page renders its own neutral palette instead of
// inheriting the CSUN crimson body background.
//
// Usage: call injectGlobalStyles() inside a useEffect with [] in the root
// component (SocialFeedPage).
//
// Exports:
//   GLOBAL_CSS           -- raw CSS string with all custom properties + animations
//   injectGlobalStyles() -- creates a <style> tag in <head> (idempotent)
//   applyTheme()         -- toggles light/dark mode via CSS custom properties
// =============================================================================
...
export const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* -- Override globals.css body background for the social feed page -------- */
  /* globals.css sets background-color: #A80532 on html,body which bleeds into
     every page. We scope-reset it here so our neutral palette takes effect
     when the social feed page is active.                                      */
  html, body {
  ...
  :root {
    ...
    /* Shadows -- layered for more realistic depth */
  ...
  /* Fade up -- used for feed cards entering the viewport */
  @keyframes fadeUp {
    from { opacity: 0; transform: translateY(18px); }
    to   { opacity: 1; transform: translateY(0); }
  }

  /* Scale in -- used for modals opening */
  @keyframes scaleIn {
    from { opacity: 0; transform: scale(0.92); }
    to   { opacity: 1; transform: scale(1); }
  }

  /* Slide down -- used for dropdown menus and popovers */
  @keyframes slideDown {
    from { opacity: 0; transform: translateY(-10px); }
    to   { opacity: 1; transform: translateY(0); }
  }

  /* Slide in from right -- used for the settings drawer panel */
  @keyframes slideInRight {
    from { opacity: 0; transform: translateX(100%); }
    to   { opacity: 1; transform: translateX(0); }
  }

  /* Shimmer -- used for image loading skeleton placeholders */
  @keyframes shimmer {
    0%   { background-position: -700px 0; }
    100% { background-position:  700px 0; }
  }

  /* Heart pop -- spring animation when liking a post */
  @keyframes heartPop {
    0%   { transform: scale(1); }
  ...
  /* Gentle pulse -- used for live/online status indicators */
  @keyframes pulse {
    0%, 100% { opacity: 1; }
    50%      { opacity: 0.5; }
  }
  ...
  /* Stagger delays so feed cards cascade in sequence */
...
// ---------------------------------------------------------------------------
// injectGlobalStyles -- call once at app root; idempotent and safe to call
// multiple times (checks for existing style tag by ID before inserting).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// applyTheme -- swaps CSS custom properties on :root to toggle between
// light and dark mode. Called whenever prefs.theme changes in SettingsPanel.
//
...
export function applyTheme(theme: 'system' | 'light' | 'dark'): void {
  ...
  if (isDark) {
    ...
    root.style.setProperty('--shadow-sm',   '0 1px 2px rgba(0,0,0,0.15), 0 1px 3px rgba(0,0,0,0.20)');
    root.style.setProperty('--shadow-md',   '0 2px 6px rgba(0,0,0,0.18), 0 4px 16px rgba(0,0,0,0.22)');
    root.style.setProperty('--shadow-lg',   '0 4px 12px rgba(0,0,0,0.20), 0 12px 40px rgba(0,0,0,0.28)');
    root.style.setProperty('--shadow-xl',   '0 8px 24px rgba(0,0,0,0.22), 0 24px 60px rgba(0,0,0,0.30)');
    ...
    root.style.setProperty('--shadow-sm',   '0 1px 2px rgba(0,0,0,0.04), 0 1px 3px rgba(0,0,0,0.06)');
    root.style.setProperty('--shadow-md',   '0 2px 6px rgba(0,0,0,0.04), 0 4px 16px rgba(0,0,0,0.06)');
    root.style.setProperty('--shadow-lg',   '0 4px 12px rgba(0,0,0,0.05), 0 12px 40px rgba(0,0,0,0.10)');
    root.style.setProperty('--shadow-xl',   '0 8px 24px rgba(0,0,0,0.06), 0 24px 60px rgba(0,0,0,0.14)');
